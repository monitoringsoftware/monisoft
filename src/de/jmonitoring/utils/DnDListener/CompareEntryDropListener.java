 ...
 * @author togro
 */
public class CompareEntryDropListener implements DropTargetListener {
        ...
        if (dtde.getDropTargetContext().getComponent().getClass() == SensorCollectionPanel.class) {
            ...
            try {
                ...
                if (id != -1) { // es ist ein Messpunkt und nicht anderes
                    HashSet<SensorProperties> sensorPropertySet = panel.getCompareValue().getPropertySet();
                    if (sensorPropertySet.size() > 0) {
                        for (SensorProperties sensorProps : sensorPropertySet) {// prüfen, ob er schon zur Gruppe gehört oder ob dieses Gebäude schon vorkommt
                            if (sensorProps.getSensorID() == SensorInformation.getSensorIDFromNameORKey(dtde.getTransferable().getTransferData(DataFlavor.stringFlavor).toString().split("\u2015")[0])
                                    || SensorInformation.getSensorProperties(sensorProps.getSensorID()).getBuildingID().equals(SensorInformation.getSensorProperties(SensorInformation.getSensorIDFromNameORKey(dtde.getTransferable().getTransferData(DataFlavor.stringFlavor).toString().split("\u2015")[0])).getBuildingID())) {
