/*
 * $Id: TreeWrapper.java,v 1.2 2011/09/26 15:46:42 togro Exp $
...
public class TreeWrapper
{
   ...
   * @param aJTree the JTree to which DnD support is added.
   */
   ...
   * @param listener a TreeTreeDnDListener to be informed of DnD events.
   */
   ...
   * @param listener a TreeTreeDnDListener not to be informed of DnD events.
   */
   ...
   * @param listener a StringTreeDnDListener to be informed of DnD events.
   */
   ...
   * @param listener a StringTreeDnDListener not to be informed of DnD events.
   */
  ...
  class PopupChooserMouseListener
  ...
  /**
   * Listens for changes in the cell renderer of the tree, and updates
   *  it with a DnDCellRendererProxy to allow for DnD borders.
   */
    ...
    implements DragGestureListener
  {
    public void dragGestureRecognized(DragGestureEvent dge)
    {
      ...
      TreePath draggedPath = tree.getClosestPathForLocation(
        dge.getDragOrigin().x, dge.getDragOrigin().y );
      if ( draggedPath == null )
        return;
      ...
      Component component = renderer.getTreeCellRendererComponent( tree,
        node, false, tree.isExpanded( draggedPath ),
        node.isLeaf(), 0, false );
...
//      tree.repaint();
    ...
    implements DropTargetListener
  {
    public void drop(DropTargetDropEvent dtde)
    {
      ...
      try
      {
        ...
        {
          dtde.dropComplete( dropNodes( ttn.getSourceTree(), ttn.getSourceNode(),
            tree, dropNode, (dtde.getDropAction() & DnDConstants.ACTION_MOVE) != 0 ) );
    ...
    private boolean dropString( String droppedString )
    {
      ...
      EventListener[] listeners =
        TreeWrapper.this.listeners.getListeners( StringTreeDnDListener.class );
    ...
    private boolean dropNodes( JTree aSourceTree, MutableTreeNode aSourceNode,
      JTree aTargetTree, MutableTreeNode aDropNode, boolean move )
    {
      // Get the mutable TreeModel
      ...
      boolean doItOurselves = true;
      EventListener[] listeners =
        TreeWrapper.this.listeners.getListeners( TreeTreeDnDListener.class );
      if( listeners != null && listeners.length > 0 )
      {
        try
        {
          TreeTreeDnDEvent event = new TreeTreeDnDEvent( aSourceTree, aSourceNode,
            aTargetTree, aDropNode );
          for( int i=0; i<listeners.length; i++ )
          {
   ...
   * @param aPath the path to drop into.
  ...
  {
    EventListener[] listeners =
      this.listeners.getListeners( StringTreeDnDListener.class );
    if ( listeners != null && listeners.length > 0 )
    {
      try
      {
        StringTreeDnDEvent event =
          new StringTreeDnDEvent( aSourceString, aTargetTree, aNode );
        for( int i=0; i<listeners.length; i++ )
        {
   ...
   *  component of the given path.
   * @param aSourceTree the source tree.
   * @param aSourceNode the source node.
   * @param aPath the path to drop into.
   * @return true to allow the drop operation, false otherwise.
   */
   ...
   * @param aSourceTree the source tree.
   * @param aSourceNode the source node.
   * @param aNode the node to drop into.
  ...
  private boolean mayDropHere( JTree aSourceTree, MutableTreeNode aSourceNode, TreeNode aNode )
  {
    boolean mayDropHere =
      ...
      ||
      ( aNode.getParent() instanceof MutableTreeNode )
      )
      &&
    ...
    if ( mayDropHere )
    {
      // Ask listeners
      EventListener [] listeners =
        this.listeners.getListeners( TreeTreeDnDListener.class );
      if ( listeners != null && listeners.length > 0 )
      {
        try
        {
  ...
  /**
   * Returns the current CustomPopupHandler being used to handle
   *   popup gestures.
   * @return the current CustomPopupHandler or null if there's no popup menu
   *   handler.
   */
   ...
   * @param customPopupHandler New value of property customPopupHandler.
   */
